(function($){  // 매개변수(파라미터 Parameter)
    ...
    const obj = {  // 객체 리터럴 방식 권장
        ...
        section1(){
            ...
            // 1. 메인슬라이드함수
            //   페이드인 // 아웃효과 애니메이션 슬라이드 구현
            //   - 모든 슬라이드를 position: absolute; 로 해준다.
            //   - 맨위 그리고 아래 포개진 슬라이드 앞뒤 순서를 정한다. z-index: 3, 2, 1
            //   - 페이드 아웃효과는 opacity: 0을 사용한다. 시간은 0.6s 또는 1s
            //   - 10개의 슬라이드는 반복적으로 회전하여 순서대로 보여준다.
            ...
            function mainNextSlide(){
                ...
                // cnt가 0이면 마지막슬라이드번호
                // 그렇지 않으면 cnt-1
                slide.eq(cnt===0? n:cnt-1).css({zIndex:3}).stop().animate({opacity: 0}, 2000);
                pageEvent();
            }
            // 메인프리뷰슬라이드(이전페이드인아웃)
            function mainPrevSlide(){
                slide.css({zIndex: 1, opacity: 1});
                slide.eq(cnt===n? n:cnt+1).css({zIndex:2});
                slide.eq(cnt).css({zIndex:3}).animate({opacity: 0}, 0).animate({opacity: 1}, 2000);
                pageEvent();
            }
            ...
            function nextCount(){
                cnt++;
                if(cnt>9) cnt=0;
                mainNextSlide(); // 메인 다음슬라이드
            }
            // 2-2. 이전카운트함수
            function prevCount(){
                cnt--;
                if(cnt<0) cnt=9;
                mainPrevSlide(); // 메인 이전슬라이드
            }
